...
stop_path = 'C:/Users/nelms/OneDrive/Penn/MUSA-508/Files/BART_stops.geojson'
...
# GET CLOSEST DISTANCES BETWEEN TWO DATAFRAMES OF POINTS AND POLYGONS
min_point_poly_distances = function(
  focus.points, # transit (BART) stops
  focus.polys   # focus area (East Bay) boundary
){
  # GET DATAFRAME OF ALL BOUNDARY POINTS
  focus.polys.exteriorpts = 
    focus.polys %>%
    st_cast(., "MULTIPOINT") %>%
    st_union() %>%
    st_cast(., "POINT")
  
  # GET VECTOR OF FOCUS POINTS
  focus.points.vector = as.vector(focus.points$geometry)
  
  # FUNCTION TO FIND THE SMALLEST DISTANCE BETWEEN 
  # A CURRENT POINT AND A VECTOR OF POINTS
  closest_distance_currentpt_vectorpts = function(current.pt, vector.pts){
    # RUN OVER POINT VECTOR TO FIND DISTANCES TO CURRENT POINT
    current_point_distances =
      sapply(vector.pts, function(vector.pt) st_distance(vector.pt,current.pt))
    # SMALLEST DISTANCE BETWEEN CURRENT POINT AND ANY VECTOR POINT
    min_current_distance = min(current_point_distances)
    return(min_current_distance)
  }
  
  # CLOSEST DISTANCES BETWEEN EVERY POLY BOUNDARY POINT AND EACH FOCUS POINT
  min_pt_poly_dists =
    # RUN THROUGH POLY POINTS
    sapply(focus.polys.exteriorpts, function(focus.polys.current.pt)
           closest_distance_currentpt_vectorpts(focus.polys.current.pt, focus.points.vector))
  return(min_pt_poly_dists)
}

# GET CLOSEST DISTANCES OF TRANSIT STOPS AND THE FOCUS AREA BOUNDARY
min_BARTstop_EBboundary_distances = 
  min_point_poly_distances(BART.stops, EB.cities %>% st_union() %>% st_convex_hull())
# GET LARGEST CLOSEST DISTANCE (MILES) BETWEEN STOPS AND BOUNDARY
max_min_BART_EB_dist = max(min_BARTstop_EBboundary_distances)/mile
...
ringbuffers_ = function(
  ...
  buffer_rings_dists = seq(from=buffer_interval, to=buffer_cap, by=buffer_interval)
  
  for (current_buffer_distance in buffer_rings_dists) {
    
    buffer.difference = 
      transit.stops %>% 
      st_buffer((current_buffer_distance-buffer_interval)*mile) %>% 
      st_union()
    ...
    if (current_buffer_distance==.5) {
      buffer.temp = st_as_sf(rbind(buffer.union, buffer.new))
    } else buffer.temp = st_as_sf(rbind(buffer.temp, buffer.union, buffer.new))
  }
...
ggplot() + 
  ...
  geom_sf(data=BART.routes, color='grey20', alpha=.5, lwd=1.5) + 
  geom_sf(data=buffer.temp[buffer.temp$stop_id=='ALL',], fill='transparent',
          aes(color=distance), lwd=.75
          ) + 
  geom_sf(data=BART.stops, fill='white',
          color='grey30', shape=21) + 
  scale_color_stepsn(
    breaks=c(0,buffer_rings_dists), 
...
BART.buffers = 
  BART.buffers[BART.buffers$Legend != 'Buffer',c('geometry')] %>%
  mutate(distance = .5)
