...
### Get Buffers of BART Stops

```{r 1A.2_Buffers}

mile = 5280

max_point_poly_distance = function(
  focus.points,
  focus.polys
){
  ...
  closest_distance_current_point_poly = function(current.polys.pt){
    current_poly_distances =
      sapply(focus.points.vector, 
             function(current.points.pt) st_distance(
               current.points.pt,
               current.polys.pt))
    min_current_distance = min(current_poly_distances)
    
    return(min_current_distance)
  }
  
  max_pt_poly_dist = max(sapply(focus.polys.exteriorpts,
                                closest_distance_current_point_poly))
  
  return(max_pt_poly_dist)
}

max_dist_stop_EB = max_point_poly_distance(BART.stops, EB.cities)
buffer_max_distance = ceiling(max_dist_stop_EB/mile)


multipleRingBuffer_transit = function(
  transit.stops,
  buffer_width=.5,
  mile_buff_cap=5,
  mile=5280
  ...
  buffer_rings_dists = seq(from=buffer_width, to=mile_buff_cap, by=buffer_width)
  
  for (current_buffer_distance in buffer_rings_dists) {
    
    buffer.difference = 
      transit.stops %>% 
      st_buffer((current_buffer_distance-buffer_width)*mile) %>% 
      st_union()
    ...
    buffer.union = 
      ...
      st_sf() %>%
      st_difference(., buffer.difference) %>%
      mutate(stop_id='ALL',stop_nm='UNION',
             route_count=0,routes='', distance=current_buffer_distance) %>%
