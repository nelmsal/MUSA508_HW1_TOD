...
tracts.all.centroid = 
  tracts.all.centroid %>%
    mutate(
      dist_BART = sapply(geometry, min_BART_dist_find))
...
tracts.all =
  ...
  merge(
    ., 
    tracts.all.centroid %>% as.data.frame() %>% select(GEOID, city, dist_BART) %>% distinct(.),
    by='GEOID'
  ) %>%
  mutate(
...
ggplot() +
  ...
  geom_sf(data=BART.buffers.TOD, fill='transparent', color='red') +
  geom_sf(
...
ggplot()+
  geom_sf(data =    EB.cities,
    fill='grey90')+
  geom_sf(data =    tracts.all, 
    aes(fill = dens_pop)) +
  geom_sf(data =    BART.buffers, 
    fill = "transparent", color = "red")+
  geom_sf(data =    BART.buffers.TOD,
    fill='transparent', color='red') + 

  scale_fill_manual(
    values = palette5,
    labels = qBr(tracts.all.group, "population"),
    name = "Population\n(Quintile Breaks, TOD in Red)") +
  labs(
    title = "Total Population 2010-2018", subtitle = "Boston, MA", caption="Figure 3.1") +
  facet_wrap(~year)+
  mapTheme() + 
...
tracts.focus = tracts.all[
  (tracts.all$year==2019) & 
    (!is.na(tracts.all[['med_rent']])),]

tracts.lm = lm(med_rent~pct_med_rent, data = tracts.focus)

print(summary(tracts.lm))

ggplot(data=tracts.focus, aes(
    x=med_rent, 
    y=pct_med_rent)) +
  geom_point() +
